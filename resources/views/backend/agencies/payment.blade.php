                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form action="{{ route('agencies.debits') }}" method="POST" id="payForm{{ $sender->id }}">
                                            ...
                                            <input type="number" name="debits" class="form-control" required>
                                    </td>
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu" aria-labelledby="paymentMethodDropdown{{ $key }}">
                                                <li><a class="dropdown-item" href="#" onclick="updatePaymentMethod('bank', '{{ $key }}', '{{ $sender->id }}')">Bank</a></li>
                                                <li><a class="dropdown-item" href="#" onclick="updatePaymentMethod('cash', '{{ $key }}', '{{ $sender->id }}')">Cash</a></li>
                                                <li><a class="dropdown-item" href="#" onclick="updatePaymentMethod('both', '{{ $key }}', '{{ $sender->id }}')">Both</a></li>
                                            </ul>
...
<!-- JavaScript Function -->
<script>
    function updatePaymentMethod(method, key, senderId) {
        const button = document.getElementById(`paymentMethodDropdown${key}`);
        const input = document.getElementById(`paymentMethod${senderId}`);
        ...
        button.textContent = labelMap[method] || 'Select Method';
        input.value = method;
    }
</script>
